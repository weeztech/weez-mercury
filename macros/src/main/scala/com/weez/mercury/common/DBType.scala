...
class dbtype extends StaticAnnotation {
  def macroTransform(annottees: Any*): Any = macro DBMacros.dbtypeImpl
}

class test(rootCollection: Boolean = true) extends StaticAnnotation {
  def macroTransform(annottees: Any*): Any = macro DBMacros.testImpl
}

class DBMacros(val c: Context) {

  import c.universe._
  import scala.reflect.runtime.{universe => ru}

  def testImpl(annottees: c.Expr[Any]*): c.Expr[Any] = {
    withFirst(annottees) {
      val tree = annottees.head.tree
      tree match {
        case q"$_ class $_(...$paramss) extends ..$_ { ..$_ }" =>
          val p = paramss(0)(0)
          println(showRaw(p))
          p match {
  ...
  def dbtypeImpl(annottees: c.Expr[Any]*): c.Expr[Any] = {
    withFirst(annottees) {
      val tree = annottees.head.tree
      withClass(tree, Some(Flag.CASE)) { (name, paramss, parents, body, mods, _, _) =>
        ...
        val caseClassParams = ensureParam(paramss, q"val id: Long")
        ...
        val companionType = tq"_root_.com.weez.mercury.common.DBObjectType[$name]" :: functionType :: Nil
        ...
        val tree = q"""
          ...
          object ${name.toTermName} extends ..$companionType {
            ...
            implicit val packer = _root_.com.weez.mercury.common.Packer(this)
          }
         """
        //println(show(tree))
        tree
  ...
  def ensureParam(paramss: List[List[Tree]], p: Tree) = {
    ...
    val arr = paramss.foldRight(Nil: List[Tree]) { (list, params) =>
      params ::: list
    }
    arr collectFirst {
